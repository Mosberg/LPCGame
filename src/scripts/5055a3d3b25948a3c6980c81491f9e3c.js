...
var CUSTOM_ANIMS = CUSTOM_ANIM_ORDER.filter((k) => CUSTOM_ANIMATIONS[k]).map(
  (key) => ({
    key,
...
var CUSTOM_ANIMS_BY_KEY = Object.fromEntries(
  CUSTOM_ANIMS.map((a) => [a.key, a]),
);
function animMeta(key) {
  ...
  if (std) {
    return {
      ...
      label: std.label,
      custom: false,
      folder: std.folder || key,
      cycle: std.cycle,
      frameCount: std.cycle.length,
      frameSize: FRAME,
    };
  }
  ...
  if (cust) {
    return {
      key,
      label: CUSTOM_ANIM_LABELS[key] || key,
      custom: true,
      def: cust.def,
      base: cust.base,
      frameCount: cust.def.frames[0].length,
      frameSize: cust.def.frameSize,
    };
  }
  return null;
}
...
function customSpritePath(base, variant) {
  return (
    "spritesheets/" +
    base +
    (variant ? variantToFilename(variant) : "") +
    ".png"
  );
}
    ...
    for (const sel of Object.values(items)) {
      ...
      for (const layer of item.l) {
        ...
        push(
          ...
          item,
          layer,
          sheetPathFor({ base, variant }, animKey),
          "std",
          lookup,
          sig,
        );
    ...
    for (const sel of Object.values(items)) {
      ...
      for (const layer of item.l) {
        ...
        push(
          ...
          item,
          layer,
          sheetPathFor({ base, variant }, meta.base),
          "extract",
          lookup,
          sig,
        );
    ...
    for (const sel of Object.values(items)) {
      ...
      for (const layer of item.l) {
        ...
        push(
          ...
          item,
          layer,
          customSpritePath(base, variant),
          def.sourceSingleAnimation ? "single" : "area",
          lookup,
          sig,
        );
      }
...
function equipmentAnimations(cat, state) {
  ...
  for (const [typeName, sel] of Object.entries(state.sel || {})) {
    ...
    const keys = [];
    for (const layer of item.l || [])
      if (layer.ca && !keys.includes(layer.ca)) keys.push(layer.ca);
    if (!keys.length) continue;
    out.push({
      type: typeName,
      itemId: sel.item,
      name: item.n,
      animations: CUSTOM_ANIMS.filter((a) => keys.includes(a.key)).map(
        (a) => a.key,
      ),
    });
  }
...
var ATTACK_BASES = /* @__PURE__ */ new Set([
  "slash",
  "thrust",
  "backslash",
  "halfslash",
...
function attackAnimations(cat, state) {
  const out = customAnimsFor(cat, state).filter((k) =>
    ATTACK_BASES.has(CUSTOM_ANIMS_BY_KEY[k].base),
  );
  const weapons = Object.values(state.sel || {})
    .map((sel) => sel && cat.items[sel.item])
    .filter(
      (item) => item && supportsBodyType(item, state.bt) && isWeaponItem(item),
    );
...
function animationMap(cat, state) {
  ...
  for (const anim of ANIMS)
    out[anim.key] = resolveAnimation(cat, state, anim.key);
  return out;
}
var MAX_FRAMES = 2400;
...
function customFrameSource(def, dirRow, frameIdx) {
  ...
  return {
    srcRow: DIR_KEY_ROWS[dir] === void 0 ? 0 : DIR_KEY_ROWS[dir],
    srcCol,
  };
}
async function getFrame(layer, dirRow, frameIdx) {
  ...
  if (layer.kind === "std") {
    ...
    srcCol = src.srcCol;
    srcFrame =
      layer.kind === "single" ? Math.max(1, Math.round(img.height / 4)) : FRAME;
  }
...
async function renderFullSheet(cat, state, onProgress) {
  ...
  for (const area of areas)
    for (const layer of area.layers) paths.add(layer.path);
  const uniq = [...paths];
  let loaded = 0;
  let cursor = 0;
  const CONC = 10;
...
async function createSprite(character, opts = {}) {
  ...
  if (opts.includeEquipmentAnims !== false) {
    for (const k of customAnimsFor(cat, st))
      if (!animKeys.includes(k)) animKeys.push(k);
    if (attack.length && !animKeys.includes(attack[0]))
      animKeys.push(attack[0]);
  }
...
async function createApp(root, host, options) {
  ...
  function animSourceNames(animKey) {
    ...
    for (const sel of Object.values(state.sel)) {
      ...
      const contributes = meta.custom
        ...
        : supportsAnim(item, animKey) ||
          (meta.base && supportsAnim(item, meta.base));
  ...
  function previewFrameIndicesFor(key) {
    ...
    if (
      meta &&
      meta.custom &&
      meta.def.skipFirstFrameInPreview &&
      idx.length > 1
    )
      return idx.slice(1);
    return idx;
      ...
      const frames = await Promise.all(
        idx.map(async (fi) => {
          const set = await Promise.all(
            layers.map((l) => getFrame(l, row, fi)),
          );
  ...
  function thumbAnimFor(item) {
    ...
    for (const layer of item.l || [])
      if (layer.ca && CUSTOM_ANIM_LABELS[layer.ca]) return layer.ca;
  ...
  function renderEquipped() {
    ...
    for (const [typeName, sel] of entries) {
      ...
      const anims = [];
      for (const layer of item.l || [])
        if (layer.ca && !anims.includes(layer.ca)) anims.push(layer.ca);
      if (anims.length) {
